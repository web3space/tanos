...
(function(){
  var requireLs, sendMedia, deleteMessage, ref$, objToPairs, join, keys, trace, express, bodyParser, cors, vm, livescript, handlebars, request, superagent, mergeImages, editMessage, makeBot, makeDbManager, create, greenlockStoreFs, unhash, ishash, getTelegramPassportText, passportScriptProxy, passportIndexProxy, passportSuccessProxy, passportCanceledProxy, proxyPassportFile, readFile, readFileSync, writeFileSync, processValidator, processValidators, processTextValidators, toString$ = {}.toString, slice$ = [].slice;
  ...
  processValidator = function(validator, text, cb){
  ...
  processValidators = function(arg$, text, cb){
    var vaidator, rest;
    vaidator = arg$[0], rest = slice$.call(arg$, 1);
    if (typeof validator == 'undefined' || validator === null) {
      return cb(null);
    }
    return processValidator(validator, text, function(err){
      if (err != null) {
        return cb(err);
      }
      return processValidators(rest, text, cb);
    });
  };
  processTextValidators = function(step, arg$, cb){
    var text, type, ref$, validate;
    ...
    validate = step.onText.validate;
    if (toString$.call(validate).slice(8, -1) === 'Array') {
      return processValidators(validate, text, cb);
    }
    if (toString$.call(validate).slice(8, -1) === 'String') {
      return processValidator(validate, text, cb);
    }
    return cb(null);
  };
  module.exports = function(arg$, cb){
    ...
    serverAddr = serverAddress + ":" + (serverSslPort != null ? serverSslPort : serverPort);
    tanos = {};
          ...
          return getText(message, function(err, text){
            ...
            return get("main:bot-step", function(err, mainStep){
              ...
              return get(previous_step + ":bot-step", function(err, previousStepGuess){
                ...
                return processTextValidators(previousStep, message, function(err){
                  if (err != null) {
                    return preventAction({
                      bot: bot,
                      chat: message.from,
                      text: "Ожидается другое значение c маской " + err
                    }, cb);
                  }
